...
class RLTrainer(BaseTrainer):
    ...
    def _setup_misc(self):
        ...
        print(self.device , torch.device(self.cfg.device) , self.cfg.device)
        self.timer = Timer()
        self._global_step = 0
    ...
    def _train_episode(self, log_every_episodes, seed_until_steps):
        ...
        episode, _ , rollouts, env_steps = self.train_sampler.sample_episode(is_train=True, render=False, random_act=seed_until_steps(ep_start_step))
    ...
    def eval(self):
        ...
        traversed_path_dict = {}
        for eval_episode_index in range(self.cfg.n_eval_episodes):
            episode, traversed_path, _, env_steps = self.eval_sampler.sample_episode(is_train=False, render=True)
            eval_rollout_storage.append(episode)
            traversed_path_dict[eval_episode_index] = traversed_path
        ...
        if self.is_chef:
            ...
            with self.logger.log_and_dump_ctx(self.global_step, ty='eval') as log:
                ...
                log('episode', self.global_episode)
                log('step', self.global_step)
        # print(traversed_path_dict)
        logger.info(traversed_path_dict)
            

        # import wandb
        # wandb.info(str(traversed_path_dict))
