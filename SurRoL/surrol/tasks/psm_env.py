...
class PsmEnv(SurRoLGoalEnv):
    ...
    def __init__(self,
        ...
        self.set_camera_view(mode="Normal")
    ...
    def _env_setup(self, cam_details = { "CAM_YAW": 90.0,
                                        "CAM_PITCH" :-30.0,
                                        "CAM_DIST" : 0.9}
    ):
        # for venv
        ...
        if self._render_mode == 'human':
            reset_camera(yaw=cam_details["CAM_YAW"], 
                         pitch=cam_details["CAM_PITCH"], 
                         dist=cam_details["CAM_DIST"]* self.SCALING,
        ...
        p.changeVisualShape(obj_id, -1,  rgbaColor=(1, 1 ,0,1))
    ...
    def set_camera_view(self, mode= "Normal"):

        if mode == "Normal":
            # Normal Front view
            self._view_matrix = p.computeViewMatrixFromYawPitchRoll(
                cameraTargetPosition=(-0.05 * self.SCALING, 0, 0.38 * self.SCALING),
                distance=0.84 * self.SCALING,
                yaw=90,
                pitch=-28,
                roll=0,
                upAxisIndex=2
            ...
            # # Leftview
            self._view_matrix = p.computeViewMatrixFromYawPitchRoll(
                cameraTargetPosition=(0.5 * self.SCALING, 0.905, 0.48 * self.SCALING),
                distance=0.76 * self.SCALING,
                yaw=9.4,
                pitch=-46,
                roll=7.3,
                upAxisIndex=2
            )
        elif mode == "left_closeup":
            # # Leftview
            self._view_matrix = p.computeViewMatrixFromYawPitchRoll(
                cameraTargetPosition=(0.5 * self.SCALING, 0.905, 0.48 * self.SCALING),
                distance=0.50 * self.SCALING,
                yaw=9.4,
                pitch=-46,
                roll=7.3,
                upAxisIndex=2
    ...
    def _step_callback(self):
        ...
        """
        # print(self.block_gripper , self.has_object ,  self._activated)
