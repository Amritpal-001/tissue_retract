...
class TissueRetract(PsmEnv):
    ...
    def _env_setup(self):
        ...
        object_location = [mesh_location[0] +0.25, mesh_location[1],  mesh_location[2]]    
        ...
        # self.anchor_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'gauze/gauze.urdf'),
        #                     object_location,
        #                     (0, 0, 0, 1),
        #                     useFixedBase=False,
        #                     globalScaling=self.SCALING*0.5)
        # obj_id = p.loadURDF(os.path.join(ASSET_DIR_PATH, 'cloth/cloth.urdf'),
        #                     object_location,
        #                     (0, 0, 0, 1),
        #                     useFixedBase=False,
        #                     globalScaling=self.SCALING*0.5)
    ...
    def _sample_goal_callback(self):
        ...
        super()._sample_goal_callback()
        self._waypoints = [None, None, None, None, None]  # five waypoints
        # pos_obj, orn_obj = self.object_location # get_link_pose(self.anchor_id, self.obj_link1)
        pos_obj = self.object_location # get_link_pose(self.anchor_id, self.obj_link1)
        # print(pos_obj, self.object_location)
        self._waypoint_z_init = pos_obj[2]

        self._waypoints[0] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.0102 + 0.005) * self.SCALING, 0., 0.5])  # approach
        self._waypoints[1] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.011) * self.SCALING, 0., 0.5])  # approach
        self._waypoints[2] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.011) * self.SCALING, 0., -0.5])  # grasp
        self._waypoints[3] = np.array([pos_obj[0], pos_obj[1],
                                       pos_obj[2] + (-0.0007 + 0.011 + 0.005) * self.SCALING, 0., -0.5])  # grasp
        self._waypoints[4] = np.array([self.goal[0], self.goal[1],
                                       self.goal[2] + 0.0102 * self.SCALING, 0., -0.5])  # lift up
        self.debug = True
        if self.debug == True:
...
if __name__ == "__main__":
    ...
    env = TissueRetract_v0(render_mode='human')  # create one process and corresponding env
