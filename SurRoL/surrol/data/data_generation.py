...
parser.add_argument('--env', type=str, default='TissueRetract-v1',
...
parser.add_argument('--render_mode', type=str, default="human",
                    help='human mode shows examples in GUI. Use "numpy" if running on a headless server')
...
def main():
    env = gym.make(args.env, render_mode=args.render_mode)  # 'human'
    num_itr = args.episodes if not args.video else 10
    cnt = 0
    ...
    while len(actions) < num_itr:
        obs = env.reset()
        print("ITERATION NUMBER ", len(actions), "/" , num_itr)
        goToGoal(env, obs)
        cnt += 1
